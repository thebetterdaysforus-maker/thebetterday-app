...
const isValidTitle = (title: string): boolean => {
  return title.trim().length >= 1 && title.length <= 40;
};
...
export default function GoalDetailScreen({ route, navigation }: any) {
  ...
  const handleSave = async () => {
    ...
    if (!isValidTitle(trimmed)) {
      Alert.alert("입력 확인", "40자 이내로 작성해 주세요.");
      return;
    }
    ...
    try {
      ...
      // 같은 날짜의 목표들만 필터링 (한국 시간대 기준 - useOptimizedGoals와 동일)
      const formatter = new Intl.DateTimeFormat('en-CA', {
        timeZone: 'Asia/Seoul'
      });
      const selectedDateLocal = formatter.format(new Date(targetTime));
      const sameDayGoals = goals.filter((g) => {
        const goalDateLocal = formatter.format(new Date(g.target_time));
        return goalDateLocal === selectedDateLocal;
      });

      console.log("📅 GoalDetailScreen 같은 날짜 필터링 (한국시간대):", {
        selectedDate: selectedDateLocal,
        selectedTime: new Date(targetTime).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
        totalGoals: goals.length,
        sameDayGoals: sameDayGoals.length,
        sameDayGoalTimes: sameDayGoals.map((g) =>
          new Date(g.target_time).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
        ),
      });
