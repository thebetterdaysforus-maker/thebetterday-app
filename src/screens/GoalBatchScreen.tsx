...
export default function GoalBatchScreen({ route }: any) {
  ...
  const onFinish = async () => {
    ...
    for (const tempGoal of tempGoals) {
      ...
      // 한국 시간대 기준으로 날짜 비교 (useOptimizedGoals와 동일)
      const formatter = new Intl.DateTimeFormat('en-CA', {
        timeZone: 'Asia/Seoul'
      });
      const tempDateLocal = formatter.format(new Date(tempGoal.time));

      // 같은 날짜의 기존 목표와만 충돌 체크 (한국 시간대 기준)
      ...
      console.log("📅 같은 날짜 목록 필터링 (한국시간대):", {
        tempGoalTitle: tempGoal.title,
        tempGoalDate: tempDateLocal,
        tempGoalTime: new Date(tempGoal.time).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
        tempGoalUTC: tempGoal.time,
        sameDayGoalsCount: sameDayGoals.length,
        sameDayGoals: sameDayGoals.map((g) => ({
          title: g.title,
          time: new Date(g.target_time).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
          date: formatter.format(new Date(g.target_time)),
          utc: g.target_time,
        })),
      });
