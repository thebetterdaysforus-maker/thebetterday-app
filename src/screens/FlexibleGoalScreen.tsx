...
const FlexibleGoalScreen: React.FC<FlexibleGoalScreenProps> = ({
  ...
  // route params에서 전달받은 날짜 사용 (오늘/내일)
  const getTargetDateString = () => {
    const { targetDate } = route.params || {};
    const result =
      targetDate === "today" ? getTodayKorea() : getTomorrowKorea();
    console.log("🎯 FlexibleGoalScreen 날짜 계산:", {
  ...
  useEffect(() => {
    // 전체 자유 목표 데이터를 가져옴 (날짜 필터링은 getGoalsByDate에서)
    fetchGoals();
  }, []);
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await fetchGoals(); // 전체 데이터 조회
    setRefreshing(false);
  };

  const renderTodayGoal = () => {
    const targetDate = getTargetDateString();
    const currentGoals = getGoalsByDate(targetDate);
    const hasGoal = currentGoals.length > 0;

    console.log("🔍 FlexibleGoalScreen 데이터 조회:", {
      targetDate: targetDate,
      currentGoals: currentGoals,
      hasGoal: hasGoal,
      storeGoals전체: useFlexibleGoalStore.getState().goals,
      조회함수결과: getGoalsByDate(targetDate),
    });
