...
const canEdit = (g: Goal, isRetrospectDone: boolean) => {
  ...
  // 🔥 현재 시간보다 미래인 목표: 편집 가능
  if (targetTime > now) {
    console.log("✅ 미래 목표 편집 가능:", {
      목표제목: g.title,
      목표시간: targetTime.toISOString(),
...
const useGoalStore = create<GoalState>((set, get) => ({
  ...
  fetchGoals: async (retryCount = 0) => {
    ...
    // 한국 시간 기준으로 최근 6개월 + 내일 목표만 가져오기 (성능 최적화)
    const koreaTime = getKoreaTime();
    const today = getTodayKorea();
    const sixMonthsAgo = new Date(koreaTime.getTime() - 180 * 86400000)
      .toISOString()
      .slice(0, 10);
    const tomorrow = new Date(koreaTime.getTime() + 1 * 86400000)
      .toISOString()
      .slice(0, 10);

      // 게스트와 인증 사용자 모두를 위한 최적화된 쿼리 (필요한 컬럼만 선택)
      const { data, error } = await supabase
        .from("goals")
        .select("id, user_id, title, target_time, status, created_at")
        .eq("user_id", session.user.id)
        .gte("target_time", sixMonthsAgo + "T00:00:00.000Z")
        .lte("target_time", tomorrow + "T23:59:59.999Z")
        .order("target_time", { ascending: true });

      // 간단한 로그만 출력 (성능 최적화)
      console.log("🚀 목표 데이터 조회 완료:", {
        개수: data?.length || 0,
        범위: `${sixMonthsAgo} ~ ${tomorrow}`,
        오류: error?.message || "없음"
      });
    ...
    console.log("📅 날짜별 목표 개수:", {
      "6개월전": getGoalCountByDate(sixMonthsAgo),
      오늘: getGoalCountByDate(today),
      "내일": getGoalCountByDate(tomorrow),
    });
    ...
    console.log("💾 goalStore 상태 업데이트:", {
      ...
      // 🔄 안정적인 재시도 로직 (배포 환경 안정성 확보)
      if (retryCount < 2) { // 최대 3회 시도
        console.log(`🔄 fetchGoals 재시도 ${retryCount + 1}/3...`);
        setTimeout(() => {
          get().fetchGoals();
        }, (retryCount + 1) * 1000); // 1초, 2초 지연
        return;
      }
      
      console.error("💥 fetchGoals 최종 실패 - 로컬 데이터 유지");
      // 재시도 실패 시 빈 배열로 설정하지 않고 기존 데이터 유지
      // set({ goals: [] });
    }
  },
  ...
  addGoal: async (title: string, target_time: string) => {
    ...
    // CustomTimePicker에서 이미 충돌 검사를 하므로 여기서는 제거
  ...
  deleteGoal: async (id: string) => {
    ...
    // 5개 제한 제거 - 18개 제한만 유지 (사용자 요청)
    // 이제 단일 목표든 배치 목표든 18개 제한만 적용됨
    ...
    // 🔕 알림 시스템 비활성화됨 - 목표 알림 정리 스킵
    console.log('🔕 알림 시스템 비활성화됨 - 목표 삭제 시 알림 정리 스킵');
  ...
  checkGoal: async (id: string) => {
    ...
    // 🔕 알림 시스템 비활성화됨 - 목표 완료 시 알림 취소 스킵
    console.log('🔕 알림 시스템 비활성화됨 - 목표 완료 시 알림 취소 스킵');
  ...
  scheduleRetrospectForLastGoal: async () => {
    ...
    if (reminderTime > nowKorea) {
      // 🔕 알림 시스템 비활성화됨 - 회고 알림 스케줄링 스킵
      console.log('🔕 알림 시스템 비활성화됨 - 회고 알림 스케줄링 스킵');
  ...
  cancelRetrospectIfLastGoalSuccess: async (goalId: string) => {
    try {
      ...
      if (successGoal && new Date(successGoal.target_time).getTime() === lastGoalTime) {
        console.log("🎉 마지막 목표 성공! 🔕 알림 시스템 비활성화됨 - 회고 알림 취소 스킵");
