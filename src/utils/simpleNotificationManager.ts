  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        await Notifications.setNotificationChannelAsync('retrospect-notifications', {
          name: 'Retrospect Notifications',
          description: '회고 작성 알림',
        ...
        if (__DEV__) console.log('📱 Android 알림 채널 설정 완료');
      }
  ...
  async requestPermission(): Promise<boolean> {
    ...
    try {
      ...
      if (existingStatus !== 'granted') {
        ...
        finalStatus = status;
        console.log('📋 알림 권한 요청 결과:', finalStatus);
      }
      
      const granted = finalStatus === 'granted';
      console.log(granted ? '✅ 알림 권한 승인됨' : '❌ 알림 권한 거부됨');
      return granted;
    } catch (error) {
  ...
  async scheduleGoalNotification(
    ...
    try {
      ...
      if (notifyTimeKorea > nowKorea) {
        ...
        console.log(`🔔 목표 알림 설정 완료:`, {
          goalId,
          title,
          알림시간: notifyTime.toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
          목표시간: targetTime.toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
          identifier: `goal_${goalId}`,
          channelId: Platform.OS === 'android' ? 'goal-notifications' : 'default'
        });
      } else {
        console.log(`⏰ 알림 시간 지남 (설정 안함): ${notifyTime.toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
      }

      console.log(`✅ 목표 "${title}" 알림 스케줄링 완료`);
    ...
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        ...
        content: {
          ...
          ...(Platform.OS === 'android' && {
            channelId: 'retrospect-notifications',
          }),
        },
        trigger: {
          type: 'date',
      ...
      console.log(`📝 회고 알림 설정:`, {
        알림시간: targetTime.toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
        identifier: 'retrospect-reminder',
        channelId: Platform.OS === 'android' ? 'retrospect-notifications' : 'default'
      });
    } catch (error) {
